...
class User {
    ...
    public static function login($request) {
        ...
        if(!empty($uid)) {
            return new RedirectResponse($request->getUriForPath('/mail/search'));
        }
    ...
    /**
     * 邮件附件上传
     * @route /admin/mail/upload
     * @access 
     * @param object 
     * @return json
     */
    public static function mailUpload($request,$filename='Filedata') {
        $savePath = '/tmp/mailupload/' . date('Ymd');
        mkdir($savePath,0700,true);   
        $file = $request->files->getParameter($filename);
        ...
        return new Response(json_encode(
                        array('status'=>'error','msg'=>'ok','path'=>'')),'200',
                        array('Content-Type'=>'application/json'));
    }
